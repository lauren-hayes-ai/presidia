...
sqlite.exec(`
  DROP TABLE IF EXISTS contact_timeline;
  DROP TABLE IF EXISTS contact_messages;
  DROP TABLE IF EXISTS meetings;
  DROP TABLE IF EXISTS contact_life_events;
...
// Clear existing data
sqlite.exec("DELETE FROM contact_timeline");
sqlite.exec("DELETE FROM meetings");
sqlite.exec("DELETE FROM contact_life_events");
sqlite.exec("DELETE FROM contact_news");
sqlite.exec("DELETE FROM career_history");
sqlite.exec("DELETE FROM contact_links");
sqlite.exec("DELETE FROM contacts");
sqlite.exec("DELETE FROM organizations");
sqlite.exec("DELETE FROM briefings");
...
const orgRows = db.select().from(schema.organizations).all();
...
const contactsData = [
  {
    ...
    organizationId: orgMap["Begin Software"],
    imageUrl: "https://ui-avatars.com/api/?name=Brandon+Frisch&background=78716c&color=fff",
    ...
    linkedinUrl: "https://linkedin.com/in/brandonfrisch",
  },
  {
    name: "Kate Simpson",
    role: "Managing Director, Head of VC",
    organizationId: orgMap["Gem Investments"],
    imageUrl: "https://ui-avatars.com/api/?name=Kate+Simpson&background=78716c&color=fff",
    ...
    linkedinUrl: "https://linkedin.com/in/katesimpson",
  },
  {
    name: "Hannah Corry",
    role: "Co-founder",
    organizationId: orgMap["Handle"],
    imageUrl: "https://ui-avatars.com/api/?name=Hannah+Corry&background=78716c&color=fff",
    ...
    linkedinUrl: "https://linkedin.com/in/hannahcorry",
  },
  {
    name: "Caroline Stevenson",
    role: "Operating Partner, Talent",
    organizationId: orgMap["Abstract"],
    imageUrl: "https://ui-avatars.com/api/?name=Caroline+Stevenson&background=78716c&color=fff",
    ...
    linkedinUrl: "https://linkedin.com/in/carolinestevenson",
  },
  {
    name: "Brandon",
    role: "Wealth Advisor",
    organizationId: orgMap["Spring Street Wealth"],
    imageUrl: "https://ui-avatars.com/api/?name=Brandon&background=78716c&color=fff",
  ...
  },
  {
    ...
    organizationId: orgMap["Inflection Capital"],
    imageUrl: "https://ui-avatars.com/api/?name=Justin+Kunz&background=78716c&color=fff",
    ...
    linkedinUrl: "https://linkedin.com/in/justinkunz",
  },
];

for (const c of contactsData) {
  db.insert(schema.contacts).values(c).run();
}

const contactRows = db.select().from(schema.contacts).all();
...
for (const l of linksData) {
  db.insert(schema.contactLinks).values(l).run();
}
...
for (const c of careerData) {
  db.insert(schema.careerHistory).values(c).run();
}
...
for (const n of newsData) {
  db.insert(schema.contactNews).values(n).run();
}
...
for (const e of lifeEventsData) {
  db.insert(schema.contactLifeEvents).values(e).run();
}

// Seed briefing
db.insert(schema.briefings).values({
  id: "2026-02-09",
...
for (const m of meetingsData) {
  db.insert(schema.meetings).values(m).run();
}
...
for (const t of timelineData) {
  db.insert(schema.contactTimeline).values(t).run();
}
